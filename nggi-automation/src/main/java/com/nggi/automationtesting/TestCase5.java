...
public class TestCase5 {
	...
	public static void main(String[] args) throws InterruptedException { 
	    ...
	    //Step 11: Verify user is able to input values in the fields which are present in "Grant Application Form" Page
	    InputValues(driver, By.id("organizationName"), "organizationName", "ATU Organization");
	    
	    // Step 1: Click on the dropdown field
	    WebElement dropdownField = driver.findElement(By.id("grantName"));
	    dropdownField.click();
	    //System.out.println("Step 1 Passed: Clicked on the dropdown field with ID 'grantName'");

	    // Step 2: Select the option with label 'Teto'
	    Select dropdown = new Select(dropdownField);
	    dropdown.selectByVisibleText("Teto");
	    System.out.println("Selected the option 'Teto' from the dropdown.");
	    InputValues(driver, By.id("budget"), "Budget", "1000");
	    InputValues(driver, By.id("projectDescription"), "Project Description", "This is something about the organization");
	    
	    //Step 12: Verify user is able to input values in the fields which are present in "Grant Application Form" Page
	    WebElement SubmitApplication_Button = driver.findElement(By.xpath("//button[contains(.,'Submit Application')]"));
	    boolean isSubmitApplication_ButtonDisplayed = SubmitApplication_Button.isDisplayed();
	    
	    if (isSubmitApplication_ButtonDisplayed == true)
  	    {
	    	SubmitApplication_Button.click();
	        System.out.println(" Step 12 Passed: The user is able to click on the Submit Button after inputting the values in the fields.");
	    }
  	    else
  	    {
  	    	System.out.println(" Step 12 Failed: The user is Not able to click on the Submit Button after inputting the values in the fields.");
  	    }
	    
	    
	    
	    // Step 13: Verify user is navigated back to the home page upon successful user login.
	    TimeUnit.SECONDS.sleep(5);
	    WebElement ApplicationSubmission_Message = driver.findElement(By.xpath("//div[@id='toast-container']/div/div[2]"));
	    boolean isApplicationSubmission_MessageDisplayed = ApplicationSubmission_Message.isDisplayed();
	   
	    String ApplicationSubmission_alertText = ApplicationSubmission_Message.getText();
	    System.out.println("Alert message: " + ApplicationSubmission_alertText);
	    
	    if (isApplicationSubmission_MessageDisplayed == true) 
	    {
	    	System.out.println("Message is Displayed");
	    	// Check if the alert message contains "Login successful"
	        if (ApplicationSubmission_alertText.contains("The application is succesfully submitted")) 
	        {
	            System.out.println(" Application Sucessfull Message Is Displayed");
	            System.out.println(" Step 13 Passed: The \"Application is submitted successfully\" message is Displayed post click on the Submit Button.");
	            SignIn_Message.click();
	        } 
	        else 
	        {
	            System.out.println(" Application Sucessfull Message Is Not Displayed");
	            System.out.println(" Step 13 Passed: The \\\"Application is submitted successfully\\\" message is Not Displayed post click on the Submit Button.");
	        }
	    } 
	    else 
	    {
